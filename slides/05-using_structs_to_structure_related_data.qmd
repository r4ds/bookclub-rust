...
* 'implements' refers to *traits* which we will learn more about later.
...
* Takes ownership but then returns the value - print values inside an expression!  
...
* Methods are like functions but defined in context struct, enum or trait object. (using an `impl` block)
...
* No need for `->` as in c++, Rust automatically dereferences as required. 
